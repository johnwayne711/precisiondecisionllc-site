...
const CODE_HELP = new Map([
  ...
  ["G4", entry("Dwell", "Pauses for the programmed dwell value.", "Retained as a commanded dwell event for bounded lathe and mill review. It contributes 0:00 to the displayed feed + rapid subtotal; its actual machine duration is outside this metric rather than treated as zero. Address interpretation remains controller-context dependent.", ALL_PATH_CONTEXTS)],
  ...
  ["M0", entry("Program stop", "Pauses the local program reader at the end of this block until the user resumes it.", "Modeled as an unconditional resumable reader event for bounded lathe and mill paths. Downstream commanded geometry remains parseable. The stop contributes 0:00 to the feed + rapid subtotal; operator-wait duration and controller-specific spindle, coolant, axis, look-ahead, or auxiliary-state effects are outside this metric rather than treated as zero.", ALL_PATH_CONTEXTS, {
    "lathe:haas-lathe-ngc": {title: "Unconditional program stop", description: "Pauses the local program reader at the end of this block until the user resumes it; the documented Haas contract also stops the main spindle and coolant.", scope: "The reader resumes at the following block and the modeled main-spindle state becomes stopped. Downstream commanded geometry remains parseable. The stop contributes 0:00 to the feed + rapid subtotal; operator-wait duration, coolant state, live-tool behavior, and other machine-specific restart effects are outside this metric rather than treated as zero."},
  })],
  ["M1", entry("Optional program stop", "Uses the visible M01 pause switch: ON pauses the local reader at the end of this block; OFF continues through it.", "Modeled for bounded lathe paths as a resumable reader choice. The stop contributes 0:00 to the feed + rapid subtotal and never converts valid downstream command geometry into PATH ONLY or dashed preview geometry. Operator-wait duration and physical spindle, coolant, axis, look-ahead, or auxiliary-state effects are outside this metric rather than treated as zero. The bounded mill reader remains unchanged while mill work is on hold.", LATHE_CONTEXTS)],
